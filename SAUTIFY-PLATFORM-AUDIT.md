...
- `OfflineBuffer.drain()` had a real concurrency bug: it read the whole queue once, then overwrote the file with only that stale snapshot's survivors, so a record `enqueue()`d while a drain was still in flight was silently discarded (reproduced directly, not just theorized). Fixed to drop each record from a fresh on-disk read immediately after its own confirmed delivery, plus a re-entrancy guard against two overlapping drains; two new regression tests cover both.
- Some §6.2 "mocked or does not exist" items were listed here as open but the site copy itself was never actually updated: `TechStack.jsx`'s false `'Sautify App (Android)'` tag (no Android app exists anywhere in this repo) removed; its "We're piloting with our first 10 venues" reworded to "onboarding our first pilot venues... targeting our first 10," since no venues are live; `ForArtists.jsx`'s "Log in to your Sautify dashboard" reworded, since no dashboard or auth exists; `ForVenues.jsx`'s `DeviceMockup` relabeled from "Sautify Device: Live" (with a pulsing "live" indicator) to "concept preview," with a plain-language caption, since it renders illustrative sample data, not a real screenshot.
